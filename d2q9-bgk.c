    ...
    for (int jj = 0; jj < params.ny; jj++)
    {
        for (int ii = 0; ii < params.nx; ii++)
        {
            ...
            ** scratch space grid */
            ...
            local_cell.speeds[0] = cells[ii + jj * params.nx].speeds[0];   /* central cell, no movement */
            local_cell.speeds[1] = cells[x_w + jj * params.nx].speeds[1];  /* east */
            local_cell.speeds[2] = cells[ii + y_s * params.nx].speeds[2];  /* north */
            local_cell.speeds[3] = cells[x_e + jj * params.nx].speeds[3];  /* west */
            local_cell.speeds[4] = cells[ii + y_n * params.nx].speeds[4];  /* south */
            local_cell.speeds[5] = cells[x_w + y_s * params.nx].speeds[5]; /* north-east */
            local_cell.speeds[6] = cells[x_e + y_s * params.nx].speeds[6]; /* north-west */
            local_cell.speeds[7] = cells[x_e + y_n * params.nx].speeds[7]; /* south-west */
            local_cell.speeds[8] = cells[x_w + y_n * params.nx].speeds[8]; /* south-east */

            if (obstacles[jj * params.nx + ii])
            {
                /* called after propagate, so taking values from scratch space
                ** mirroring, and writing into main grid */
                tmp_cells[ii + jj * params.nx].speeds[1] = local_cell.speeds[3];
                tmp_cells[ii + jj * params.nx].speeds[2] = local_cell.speeds[4];
                tmp_cells[ii + jj * params.nx].speeds[3] = local_cell.speeds[1];
                tmp_cells[ii + jj * params.nx].speeds[4] = local_cell.speeds[2];
                tmp_cells[ii + jj * params.nx].speeds[5] = local_cell.speeds[7];
                tmp_cells[ii + jj * params.nx].speeds[6] = local_cell.speeds[8];
                tmp_cells[ii + jj * params.nx].speeds[7] = local_cell.speeds[5];
                tmp_cells[ii + jj * params.nx].speeds[8] = local_cell.speeds[6];
            }
            else
            {
                ...
                {
                    local_density += local_cell.speeds[kk];
                }
                ...
                for (int kk = 0; kk < NSPEEDS; kk++)
                {
                    tmp_cells[ii + jj * params.nx].speeds[kk] = local_cell.speeds[kk] + params.omega * (d_equ[kk] - local_cell.speeds[kk]);
                }
            }
        }
    }
